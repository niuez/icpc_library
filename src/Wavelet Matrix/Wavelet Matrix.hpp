class bitvector {
  using bit_type = uint_least64_t;
  using size_type = size_t;
  static constexpr size_type wordsize = 64;
  
  vector<bit_type> bit;
  vector<size_type> sum;
...
class wavelet_matrix {
  ...
  vector<size_type> spl;

public:

  wavelet_matrix(const vector<integer_type>& arr, size_type de)
    : depth(de),
      len(arr.size()),
      mat(vector<bitvector>(depth, bitvector(arr.size()))),
      spl(vector<size_type>(depth, 0)) {
        vector<size_type> idx(len);
        vector<size_type> left(len), right(len);
