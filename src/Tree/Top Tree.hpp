namespace toptree {
  ...
  struct cluster {
    ...
    cluster() {}
    cluster(int a, int b): sub(a), sub_cnt(b) {}
    ...
    static size_t select(const cluster& a, const cluster& b, V av, V bv, V cv) {
      return 0;
    }
  };
  ...
  using vertex_index = uint_least32_t;
  ...
  static void debug_tree(node_index i, string ind) {
    if(!i) return;
  ...
  inline void additive(node_index i, int x);
